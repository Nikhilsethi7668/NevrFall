        ...
        <div className="navbar-start">
          <div className="drawer md:hidden lg:hidden">
            ...
            <div className="drawer-side">
              ...
              <div className="bg-base-100 min-h-full w-80 p-4 z-30 flex flex-col">
                <div className="flex justify-between items-center mb-4">
                  <Link href="/" className="drawer__logo">
                    <span className="text-xl font-bold">NevrFall</span>
                  </Link>
                ...
                <ul className="flex-grow space-y-1 z-30 overflow-y-auto">
                  {collections.map((collection: any) => (
                    <li key={collection._id}>
                      <Link href={`/products?collections=${collection.slug || collection.name}`} onClick={closeDrawer} className="mobile-navlink text-[10px]">
                        {collection.name}
                      </Link>
                    </li>
                  ))}
                  ...
                  <li>
                    <button onClick={() => setOpenCategories(!openCategories)} className="mobile-navlink text-[10px] w-full text-left flex justify-between items-center">
                      SHOP BY CATEGORY
                    ...
                    {openCategories && (
                      <div className="flex flex-col gap-2 p-2">
                        {categories.map((category) => (
                          ...
                          >
                            <h3 className='py-1 px-2 text-[10px]'>
                              {category.name}
                            </h3>
                          </div>
                        ))}
                  ...
                  <li><Link href="/contact" onClick={closeDrawer} className="mobile-navlink text-[10px]">CONTACT US</Link></li>
                  <li><Link href="/return" onClick={closeDrawer} className="mobile-navlink text-[10px]">EXCHANGE</Link></li>

                  {isLoggedIn && (
                    <div className='mt-40 fixed bottom-1/2 w-72'>
                      <hr />
                      ...
                      <li>
                        <Link
                          ...
                          className="mobile-navlink text-[10px]"
                        >
                          My Profile
                        </Link>
                      </li>
                ...
                <div className="mt-4">
                  ...
                  {isLoggedIn ? (
                    <ul className="space-y-2">
                      <li><button onClick={() => { handleLogout(); closeDrawer(); }} className="btn btn-error w-full">Log out</button></li>
                    </ul>
                  ) : (
                    <ul className="space-y-2">
                      <li onClick={closeDrawer}><LoginDialog open={open} setOpen={setOpen} /></li>
                    </ul>
                  )}
                </div>
              </div>
          ...
          <Link href="/" className="text-lg sm:text-xl font-semibold uppercase tracking-wide text-base-content">NevrFall</Link>
        </div>
